...
const INITIAL_MEALS: Meal[] = [
  {
    ...
    ingredients: [],
    instructions: [
      'Brew black coffee (no milk, sugar, or cream).',
...
function mealToForm(meal: Meal): MealFormState {
  return {
    ...
    emoji:        meal.emoji,
    tags:         meal.tags.join(', '),
    prepTimeMins: String(meal.prepTimeMins ?? ''),
    cookTimeMins: String(meal.cookTimeMins ?? ''),
...
function formToMeal(form: MealFormState, id: string): Meal {
  return {
    id,
    name:         form.name.trim(),
    emoji:        form.emoji || '🍽️',
      ...
      <div className="app-card" style={{
        ...
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontFamily: 'DM Sans, sans-serif', fontWeight: 700, fontSize: '0.9rem', color: 'var(--charcoal)', marginBottom: '0.25rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {meal.name}
            </div>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
              {meal.macros.carbs > 0 && <span className="macro-tag carbs">{meal.macros.carbs}g C</span>}
              {meal.macros.protein > 0 && <span className="macro-tag protein">{meal.macros.protein}g P</span>}
              {totalMins > 0 && (
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.25rem' }}>
        <button className="btn-app-primary" style={{ flex: 1 }} onClick={onEdit}>Edit Meal</button>
        <button className="btn-app-secondary" onClick={onDelete}
      ...
      {meal.macros.calories > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.5rem', background: 'var(--app-bg)', borderRadius: 12, padding: '0.875rem', marginBottom: '1.25rem' }}>
          {[
            { label: 'Cal',     value: meal.macros.calories, unit: '',  color: undefined },
            ...
            <div key={m.label} style={{ textAlign: 'center' }}>
              <div style={{ fontFamily: 'Playfair Display, serif', fontSize: '1.1rem', fontWeight: 900, color: m.color ?? 'var(--charcoal)' }}>
                {m.value}{m.unit}
              </div>
      ...
      {meal.ingredients.length > 0 && (
        <div style={{ marginBottom: '1rem' }}>
          ...
          {meal.ingredients.map((ing, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.4rem 0.625rem', background: 'var(--app-bg)', borderRadius: 8, fontSize: '0.85rem', fontFamily: 'DM Sans, sans-serif', marginBottom: '0.25rem' }}>
...
function MealForm({ form, setForm, onSave, onCancel, isNew }: {
  form: MealFormState
  setForm: React.Dispatch<React.SetStateAction<MealFormState>>
  onSave: () => void
  onCancel: () => void
  isNew: boolean
}) {
  ...
  const updateStep = (idx: number, val: string) =>
    setForm(f => { const s = [...f.instructions]; s[idx] = val; return { ...f, instructions: s } })
  const addStep = () => setForm(f => ({ ...f, instructions: [...f.instructions, ''] }))
  const removeStep = (idx: number) => setForm(f => ({ ...f, instructions: f.instructions.filter((_, i) => i !== idx) }))
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '3rem 1fr', gap: '0.5rem', marginBottom: '1rem' }}>
        <div>
          ...
          <input className="app-input" value={form.emoji} maxLength={2}
        ...
        <div>
          ...
          <input className="app-input" value={form.name} placeholder="e.g. Mince Taco Bowl"
      ...
      <div className="form-group">
        ...
        <input className="app-input" value={form.tags} placeholder="keto, mince, quick"
      ...
      <div style={{ background: 'var(--app-bg)', borderRadius: 12, padding: '0.875rem', marginBottom: '1rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.5rem' }}>
          {(['calories', 'protein', 'fat', 'carbs'] as const).map(key => (
            <div key={key}>
              <label className="app-label" style={{ fontSize: '0.55rem' }}>{key}</label>
              <input className="app-input" type="number" min="0"
      ...
      <div style={{ marginBottom: '1rem' }}>
        ...
        {form.ingredients.length === 0 && (
          <p style={{ fontSize: '0.8rem', color: 'var(--text-subtle)', fontFamily: 'DM Sans, sans-serif', fontStyle: 'italic' }}>
            No ingredients yet
          </p>
        )}
      ...
      <div style={{ marginBottom: '1.25rem' }}>
        ...
        {form.instructions.map((step, idx) => (
          <div key={idx} style={{ display: 'grid', gridTemplateColumns: '1.5rem 1fr auto', gap: '0.375rem', marginBottom: '0.375rem', alignItems: 'center' }}>
            ...
            <input className="app-input" value={step} placeholder={`Step ${idx + 1}`}
              style={{ fontSize: '0.8rem', padding: '0.4rem 0.625rem' }}
...
export default function MealsPage() {
  ...
  const handleSave = (onClose: () => void) => {
    ...
    const updated = formToMeal(form, id)
    if (editingId) {
      setMeals(prev => prev.map(m => m.id === editingId ? updated : m))
    } else {
      setMeals(prev => [...prev, updated])
    }
    setForm(EMPTY_FORM)
    setEditingId(null)
    onClose()
  ...
  const handleDelete = (id: string, onClose: () => void) => {
    if (!confirm('Delete this meal?')) return
    setMeals(prev => prev.filter(m => m.id !== id))
    onClose()
  }

  const renderListItem = (meal: Meal, isSelected: boolean, onSelect: (m: Meal) => void) => (
    <MealCard key={meal.id} meal={meal} isSelected={isSelected} onSelect={(m) => { onSelect(m); handleSelect(m) }} />
  )
  ...
  const renderDetail = (
    ...
    if (mode === 'view' && selected) {
      const meal = meals.find(m => m.id === selected.id) ?? selected
      return (
        <MealDetail
          meal={meal}
          onEdit={() => { handleSelect(meal); setMode('edit') }}
          onDelete={() => handleDelete(meal.id, onClose)}
        />
      )
    }
    return (
