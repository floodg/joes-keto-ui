...
const TIME_EMOJIS = { breakfast: '☀️', lunch: '🌤️', dinner: '🌙', snack: '⚡' }
const TIME_LABELS = { breakfast: 'Breakfast', lunch: 'Lunch', dinner: 'Dinner', snack: 'Snack' }
const DAYS = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']

// Mock meals catalog
const MEALS_CATALOG = [
  { id: 'bw', name: 'Black Coffee / Water', macros: { fat: 0, protein: 0, carbs: 0 }, emoji: '☕' },
  { id: 'mt', name: 'Mince Taco Bowl', macros: { fat: 30, protein: 55, carbs: 8 }, emoji: '🌮' },
  { id: 'sg', name: 'Steak + Greens', macros: { fat: 22, protein: 48, carbs: 4 }, emoji: '🥩' },
  { id: 'ss', name: 'Salmon Salad', macros: { fat: 18, protein: 38, carbs: 5 }, emoji: '🐟' },
  { id: 'ca', name: 'Chicken + Avocado Salad', macros: { fat: 20, protein: 42, carbs: 4 }, emoji: '🥗' },
  { id: 'mb', name: 'Mince Bowl', macros: { fat: 28, protein: 52, carbs: 3 }, emoji: '🥩' },
  { id: 'sa', name: 'Salmon + Avocado Salad', macros: { fat: 22, protein: 36, carbs: 3 }, emoji: '🍣' },
  { id: 'dt', name: 'Daily Targets', macros: { fat: 0, protein: 0, carbs: 0 }, emoji: '🎯' },
]

// Build initial week plan
...
export default function WeeklyPlan() {
  ...
  const [addingSlot, setAddingSlot] = useState(null) // { dayIdx, time }
  ...
  const getMeal = (mealId) => MEALS_CATALOG.find(m => m.id === mealId)

  const toggleStatus = (dayIdx, time) => {
    setPlan(prev => {
      ...
      return {
        ...
        [dayIdx]: {
          ...
          [time]: {
            ...slot,
            status: slot.status === 'completed' ? 'planned' : 'completed',
          },
        },
      }
    })
    ...
    <div>
      ...
      <div className="page-header-bar">
        ...
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
          <button className="btn-app-ghost" onClick={prevWeek}><ChevronLeft size={18} /></button>
          <button className="btn-app-secondary" onClick={goToday} style={{ fontSize: '0.8rem', padding: '0.375rem 0.875rem' }}>
            Today
          </button>
          <button className="btn-app-ghost" onClick={nextWeek}><ChevronRight size={18} /></button>
        </div>
      </div>
            ...
            return (
              <div key={i} style={{
                ...
                <div style={{
                  ...
                  color: isToday ? '#efaa52' : 'rgba(255,255,255,0.5)',
                ...
                <div style={{
                  fontSize: '0.6rem', fontFamily: 'DM Sans, monospace',
                  color: carbs > 25 ? '#fca5a5' : 'rgba(255,255,255,0.35)',
                  marginTop: '0.125rem',
                }}>
                  {carbs}g carbs
                </div>
          ...
          {MEAL_TIMES.map(time => (
            <>
              ...
              <div key={`label-${time}`} style={{
                ...
                <span style={{
                  fontSize: '0.55rem', fontFamily: 'DM Sans, sans-serif', fontWeight: 700,
              ...
              {weekDates.map((date, dayIdx) => {
                ...
                const meal = slot ? getMeal(slot.mealId) : null
                const isToday = isSameDay(date, today)
                  ...
                  <div key={`${dayIdx}-${time}`} style={{
                    ...
                    {meal ? (
                      <div style={{
                        ...
                        background: slot.status === 'completed' ? '#f0fdf4'
                          : slot.status === 'skipped' ? '#f9fafb' : '#eff6ff',
                        ...
                        <button
                          ...
                          style={{
                            ...
                            padding: 2, borderRadius: 4, display: 'none',
                          }}
                        ...
                        {slot.status === 'completed' && (
                          <span style={{
                            fontSize: '0.6rem', color: '#15803d', fontWeight: 700,
      ...
      <style>{`
        ...
        div:hover > .remove-btn { opacity: 1; display: flex !important; }
      `}</style>
    </div>
  )
}
