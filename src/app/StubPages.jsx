...
function ComingSoon({ emoji, title, description, links = [] }) {
  return (
    <div>
      <div className="page-header-bar">
        <div>
          <h1 className="page-title">{emoji} <em>{title}</em></h1>
        </div>
      </div>
      <div className="app-card" style={{ padding: '3rem 2rem', textAlign: 'center' }}>
        <div style={{ fontSize: '4rem', marginBottom: '1rem' }}>{emoji}</div>
        <h2 style={{ fontFamily: 'Playfair Display, serif', fontSize: '1.5rem', color: 'var(--charcoal)', margin: '0 0 0.75rem' }}>
          {title}
        </h2>
        <p style={{ fontFamily: 'DM Sans, sans-serif', color: 'var(--text-muted)', fontSize: '0.95rem', maxWidth: 400, margin: '0 auto 1.5rem' }}>
          {description}
        </p>
        {links.length > 0 && (
